...
:breadcrumbs="[
    ...
    <form method="POST" action="{{route('adminPrestamo.update',$prestamo->PK_Prestamos)}}" class="submit-form"
         target="_blank" onsubmit="setTimeout(() => { window.location.href='{{ route('adminbien.index') }}'; }, 2500);"> 
        @csrf
        @method('PUT')
        ...
        <div class="hidden" >
            {{-- codigo patrimonial --}}
            <input name="id" type="text" id="id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ingrese el codigo unitario de 12 digitos"  value="{{$id}}"/>
            <input name="FK_Prestamo_HardwareId" type="text" id="FK_Prestamo_HardwareId" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ingrese el codigo unitario de 12 digitos"  value="{{$prestamo->bien->UK_Hardware_Codigo}}"/>
        </div>
        ...
        <div class="grid gap-2 mb-4 md:grid-cols-2 ">
                ...
                <div>
                    ...
                    <input name="Tdoc_ref_prestamo" type="text" id="usuario" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" maxlength="30"  required  placeholder="Escriba el documento el cual tendra relacion con el acta"  value="{{$prestamo->Tdoc_ref_prestamo}}"/>
        ...
        <div class="grid gap-6 mb-4 md:grid-cols-3">
            <div>
                ...
                <input name="usuario" type="text" id="usuario" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" maxlength="30"  required  placeholder="Nombre y apellidos del usuario"  value="{{$prestamo->Tresponsable_prestamo}}"/>
                @error('usuario')
                        <p class="text-red-600">*{{$message}}</p>
                @enderror
            </div>
                ...
                <select required name="FK_Prestamo_AreaId" id="miSelect-area" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value=""selected disabled >---Seleccioné un área---</option>
                        @foreach ($areas as $area)
                            @if ($area->PK_area != 1 )
                                <option value="{{$area->PK_area}}" {{ $prestamo->area->PK_area == $area->PK_area ? 'selected': '' }} >{{$area->UK_Nombre_area}}</option>
                            @endif
                        @endforeach
                </select>
        ...
        //que seleciona todos esos formularios que tengan ese nombre de delete-form 
            ...
            forms.forEach(form => {
                ...
                form.addEventListener('submit',function(e){ //e es el evento en si
                    ...
                    e.preventDefault('');
                        Swal.fire({
                            title: "Estas seguro de Editar esta reparacion?",
                            text: "No podras revertir esto!",
                            icon: "warning",
                            showCancelButton: true,
                            confirmButtonColor: "#3085d6",
                            cancelButtonColor: "#d33",
                            confirmButtonText: "Si, Editar reparacion",
